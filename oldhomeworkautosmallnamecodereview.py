...
def oldhomeworksmallnamereviewHtml(rows, cursor, hw_num):
    # 標題
    table = HTMLTable()
    # 表頭行
    ...
    cursor_list = sorted(cursor, key=lambda x:x["Number"])
    for dic in cursor_list:
        idx = html.index("未")

        if "Time_file" in dic:
            html[idx:idx+3] = [f"<a href='/oldhomeworkcodepage/Time/{hw_num}/{str(dic['_id'])}'>程式碼</a>"]
        else:
        ...
        if "Memory_file" in dic:
            html[idx+87-2:idx+87+3-2] = [f"<a href='/oldhomeworkcodepage/Memory/{hw_num}/{str(dic['_id'])}'>程式碼</a>"]
        else:
    ...
    <body style='background-color: #37464a;'>
        <table border="3" width="300px" height="100px" style="border-color:#555858;">
            <tr>
                <td bgcolor="#37464a">
                    <form action="/oldhomeworkcodereviewsheet/{hw_num}" style="display:inline-block;"><button class="but"style="font-size: 23px;">查看審查名單</button></form>
    ...
    # print(os.getcwd())
    # # print("--------------------------------")
    with open(f"templates/oldhomework{hw_num}codereview.html", "w", encoding="utf-8") as file:
