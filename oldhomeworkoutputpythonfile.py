...
class OldPythonToHTML:
    ...
    def msg_style(self):
        style =  f"""
                    ...
                    input{{outline-style: none ;border: 5px solid #ccc; border-radius: 3px; border-color: #965ca1;padding: 14px 14px;width: 500px;font-size: 20px;font-family: consolas;}}
    ...
    def alert_style(self):
        ...
        alert_style = """
            ...
            .modal-content {background-color: #fefefe; margin: auto; padding: 10px; border: 1px solid #888; width: 20%;}
    ...
    def to_html_msg_board(self):
        ...
        if result != None:
            ...
            for i in total:
                ...
                text += f"""<tr>
                                <td style="border-color:#000;border-width:1px;border-style:solid;padding:5px;background-color: white;">{i[0]}</td>
                                <td style="border-color:#000;border-width:1px;border-style:solid;padding:5px;background-color: white;">{i[1]}</td>
                                <td style="word-wrap:break-word;border-color:#000;border-width:1px;border-style:solid;padding:5px;background-color: white;">{i[2]}<br/></td>
                            </tr>
                            """
    ...
    def to_html_menu(self):
        htmltext = f"""
                    <table border="3" width="300px" height="100px" style="border-color:#555858;" class="tabbut">
                        <tr style="text-align: center;">
                            <td bgcolor="#37464a">
                                <form action="/oldhomeworkcodereview/{self.hwn}" style="display:inline-block;"><button class="but" style="font-size: 23px;">程式碼審查清單</button></form>
                                <form action="/oldhomeworkmember/{self.hwn}" style="display:inline-block;"><button class="but" style="font-size: 23px;">返回會員首頁</button></form>
                            </td>
                        </tr>
                    </table>
                    """
...
def old_read_python_file(filename, id, hwn, Name, bol, collection, Type, ojid, number): 
    now = os.getcwd()
    pth = OldPythonToHTML(f"{filename}.py", hwn, id, Name, bol, collection, Type, ojid, number)
    html = pth.main()
    #-----------------------------------------------------#
    os.chdir("./templates")
    sys.dont_write_bytecode = True
    sys.path.append(os.getcwd())
    #-----------------------------------------------------#
    with open(f"oldhomeworkcodepage.html", "w", encoding="utf-8") as f:
        f.write(html)
        os.chdir(now)