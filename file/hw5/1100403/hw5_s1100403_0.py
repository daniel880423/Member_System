def homework_5(matrix, start, end, total): # 請同學記得把檔案名稱改成自己的學號(ex.1104813.py)
    ...
    return [D[start-1][end-1],str(start)+Path(start,end,P,traval)+str(end)]

def Path(start,end,P,traval):
    if P[start-1][end-1] != -1:
        Path(start,P[start-1][end-1],P,traval)
        traval = traval + str(P[start-1][end-1]+1)
        Path(P[start-1][end-1],end,P,traval)
    return traval
