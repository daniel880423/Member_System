def homework_5(matrix, start, end, total): # 請同學記得把檔案名稱改成自己的學號(ex.1104813.py)
    ...
    if start == end: 
        return[-1, None]
    W = [] #matrix[i](a, b, c)  W[a][b]將儲存c的值
    path = []
    for i in range(0,total+1):
        W.append([float("inf")]*(total+1)) #建立一個(total+1)*(total+1)階的W矩陣且全部的值都是無限大
        path.append([-1]*(total+1)) #建立一個(total+1)*(total+1)階的path矩陣且全部的值都是-1
    for i in range(1,total+1):
        W[i][i] = 0 #任何節點到自己所需的距離都為0

    for i in matrix:
        W[i[0]][i[1]] = i[2] #將matrix給予的值存入W

    A = W #複製W矩陣  且A將儲存最短路徑的值
