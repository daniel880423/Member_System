def homework_5(matrix, start, end, total): 
    ...
    ans = [W[start][end], ""]                               #ans[0]=start到end的最短距離 ans[1]=路徑
    if ans[0] == float("inf"):                              #ans[0]=inf 也就是並無start到end的路徑就直接回傳[-1, None] 
        return[-1, None] 

    ans[1] = str(start)                                     #建立start到end的起點start
    def getpath(i, j):                                      #尋找(i, j)之中的中轉點
        if (P[i][j] != -1):                                 #中轉點!=-1 代表存在中轉點path[i][j]
            getpath(i, P[i][j])                             #尋找(i, 中轉點path[i][j])之中的中轉點
            ans[1] +=str(P[i][j])                           #路徑加上中轉點
            getpath(P[i][j], j)                             #尋找(中轉點path[i][j], j)之中的中轉點
    getpath(start, end) 
    ans[1] += str(end)
