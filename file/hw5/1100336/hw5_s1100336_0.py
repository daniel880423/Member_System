def homework_5(matrix, start, end, total): # 請同學記得把檔案名稱改成自己的學號(ex.1104813.py)
    ...
    path=[['']*total for i in range(0,total)]  #設一個矩陣存經過的節點
       ...
       for i in range(total):
           for j in range(total):
               if (D[i][k]+D[k][j]<D[i][j]):
                    ...
                    path[i][j]=path[i][k]+path[k][j]+str(k+1)  #把經過的節點存入矩陣
    
    if D[start-1][end-1]==float("inf"):   #如果經過以上的Floyd演算法仍是無限大的步數
        D[start-1][end-1]=-1       #就把該點設成-1
        re_path=None       #要回傳經過的節點的矩陣的那格輸出None
    else:
        re_path=str(start)+path[start-1][end-1]+str(end)  #不是無限大就回傳包刮起使和結束和經過的節點合併的字串
