def homework_9(bag_size, items): # 請同學記得把檔案名稱改成自己的學號(ex.1104813.py)
    ...
    def bfs(queue,visit,total_visit,max_price):
        size=queue[0][0]
        price=queue[0][1]
        v=queue[0][2]
        queue.pop(0)
        for i in range(1,len(items)):
            if size+items[i][0]<bag_size  :#and price+items[i][1]>max_price
                new_size=size+items[i][0]
                new_price=price+items[i][1]
                max_price=max(max_price,new_price)
                v.add(i)
                queue.append([new_size,new_price,v])
                total_visit.append(v)
                #bfs(queue,items[i],total_visit,max_price)
        return max_price

    ans=bfs(queue,visit,total_visit,max_price)
