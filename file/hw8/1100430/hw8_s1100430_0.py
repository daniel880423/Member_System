...
def quarrel(check, nextX):                                                         #定義function quarrel判斷與現存皇后衝突的位置(意即為promising)，參數為tuple及下個位置
    nextY = len(check)                                                             #依列判斷
    con = any(abs(check[i] - nextX) in (0, nextY - i) for i in range(nextY))       #判斷是否同列或同對角線(nextY - i:x座標絕對值之差 = y座標絕對值之差)
    return con                                                                     #回傳布林值
