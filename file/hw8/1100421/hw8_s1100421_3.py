...
def dfs(row, N):
    ...
    while col[row] <= N-1 and row<= N-1:         
        if jud(row):            #設立此方法來驗證是否為可設立皇后之節點
    ...
    if col[row] == N-1 or col[row] == N:               #此條件成立，代表這個row的所有column都已經走訪完且沒有找到適合的點，將col[row]重置，並往上一個row繼續走訪
        col[row] = 0
        return

def jud(row):              #判斷此節點若是設置皇后會不會被攻擊
    if row == 0:
        return True
    else:
        for i in range(row):
...
def ans(N):                #將得到的其中答案取出並轉換格式儲存至全域變數的lst中
    tmp_lst= []
    for i in range(N):
        a="-"
        b=a*col[i]
